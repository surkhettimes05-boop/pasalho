...
export class InventoryLedgerService {
   ...
   */
  static assertSnapshotWillNotGoNegative(
    currentBase: number,
    delta: number,
  ): void {
  ...
  static assertBatchRequirements(
  ...
  async postEvent(
    ...
    if (input.idempotencyKey) {
      const existing = await db.inventoryEvent.findUnique({
        where: { idempotencyKey: input.idempotencyKey },
      });
      if (existing) return { eventId: existing.id };
    }

    const post = async (tx: Prisma.TransactionClient) => {
    ...
    const eventId = transaction
      ...
      : await this.prisma.$transaction(post);
  ...
  private async applyMovementWithinTransaction(
    ...
    >`
      SELECT id, "baseQuantity" AS base_quantity, quantity, version
      FROM "InventorySnapshot"
      WHERE "locationId" = ${m.locationId}
        AND "productId" = ${m.productId}
        AND "batchId" IS NOT DISTINCT FROM CAST(${m.batchId ?? null} AS TEXT)
  ...
  async validateAndLockStock(
    ...
    const snapshots = await tx.$queryRaw<
      Array<{ id: string; base_quantity: string }>
    >`
      SELECT id, "baseQuantity" AS base_quantity
      FROM "InventorySnapshot"
      WHERE "locationId" = ${locationId}
        AND "productId" = ${productId}
        AND "batchId" IS NOT DISTINCT FROM CAST(${batchId ?? null} AS TEXT)
